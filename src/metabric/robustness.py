...
def horizon_cohort(raw: pd.DataFrame, months: int):
    """Patients with observable horizon status, with 1 = death before horizon."""
...
def threshold_and_calibration(model, X_train, y_train, X_test, y_test, figure_dir: Path) -> dict:
    """Threshold table and calibration of predicted mortality risk."""

    p_death = model.predict_proba(X_test)[:, 1]
    death = y_test.to_numpy()

    rows = []
    for threshold in THRESHOLDS:
        flagged = (p_death >= threshold).astype(int)
        rows.append({
            "death_probability_threshold": threshold,
            "death_recall": float(recall_score(death, flagged)),
            "death_precision": float(
                precision_score(death, flagged, zero_division=0)
            ),
            "flagged_share": float(flagged.mean()),
        })

    frac_pos, mean_pred = calibration_curve(
        death, p_death, n_bins=8, strategy="quantile"
    )

    logit = np.log(
        np.clip(p_death, 1e-4, 1 - 1e-4)
        / (1 - np.clip(p_death, 1e-4, 1 - 1e-4))
    )

    from sklearn.linear_model import LogisticRegression

    fit = LogisticRegression(C=1e6).fit(
        logit.reshape(-1, 1), death
    )
    ...
    calibrator = fit_platt(oof, y_train)
    calibrated = apply_platt(calibrator, p_death)
    ...
    return {
        ...
        "recalibrated_brier": float(
            brier_score_loss(y_test, calibrated)
        ),
    }
...
def _death_operating_points(y_test, p_death) -> dict:
    """Death-class recall/precision at the default threshold and matched flagged share."""

    death = np.asarray(y_test).astype(int)
    p_death = np.asarray(p_death)
    ...
    out = {
        ...
        "brier": brier_score_loss(death, p_death),
        "recall_at_0.5": recall_score(death, default),
        "flagged_share_at_0.5": default.mean(),
    }

    for target in (0.5, 0.8):
        out[f"precision_at_recall_{target}"] = float(
            precision[recall >= target].max()
        )

    # Recall when flagging approximately the observed 5-year mortality share.
    top_k = int(round(death.mean() * len(death)))

    flagged = np.zeros(len(death), dtype=int)
    flagged[np.argsort(-p_death)[:top_k]] = 1

    out["recall_flagging_observed_mortality_share"] = recall_score(
        death, flagged
    )
...
def _calibration_slope_intercept(y_test, p_death) -> tuple[float, float]:
    """Slope and intercept of mortality outcome on logit predicted mortality."""
...
def calibration_comparison(X, y, genomic, n_splits: int = 20) -> dict:
    ...
    for seed in range(n_splits):
        ...
        for name, pipeline in (
            ("random_forest", fresh_pipeline(genomic, n_estimators=200)),
            ("xgboost", xgboost_pipeline(genomic)),
        ):
            model = clone(pipeline).fit(X_fit, y_fit)

            p_cal_death = model.predict_proba(X_cal)[:, 1]
            p_raw_death = model.predict_proba(X_te)[:, 1]
            ...
            iso = IsotonicRegression(
                ...
                y_min=0,
                y_max=1,
            ).fit(p_cal_death, y_cal)

            p_platt_death = platt.predict_proba(
                logit(p_raw_death)
            )[:, 1]

            p_iso_death = iso.predict(p_raw_death)

            slope, intercept = _calibration_slope_intercept(
                y_te,
                p_raw_death,
            )

            slope_p, intercept_p = _calibration_slope_intercept(
                y_te,
                p_platt_death,
            )
            ...
            records[name].append({
                ...
                "auc_platt": roc_auc_score(
                    y_te, p_platt_death
                ),
                "slope_raw": slope,
                "intercept_raw": intercept,
                "slope_platt": slope_p,
                "intercept_platt": intercept_p,
                "mean_predicted_mortality_raw": float(
                    np.mean(p_raw_death)
                ),
                "mean_predicted_mortality_platt": float(
                    np.mean(p_platt_death)
                ),
    ...
    for name, rows in records.items():
        summary[name] = {
            key: {
                ...
                "std": float(
                    np.std([r[key] for r in rows], ddof=1)
                ),
            }
            for key in rows[0]
        }
    ...
    for key in (
        ...
        paired[key] = {
            ...
            "xgboost_minus_rf_std": float(
                diff.std(ddof=1)
            ),
            "xgboost_better_in_splits": int(
                (diff < 0).sum()
            ),
        }
...
def _survival_model_comparison(
    raw: pd.DataFrame,
    n_splits: int,
    horizon: int,
) -> dict:
    """Penalised Cox vs the 5-year mortality classifier on the same test patients."""
    ...
    time = raw["overall_survival_months"].to_numpy(
        dtype=float
    )

    event = (
        1 - raw["overall_survival"]
    ).to_numpy()
    ...
    for seed in range(n_splits):
        ...
        Z_tr = preprocessor.transform(
            X_all.iloc[idx_tr]
        )
        Z_te = preprocessor.transform(
            X_all.iloc[idx_te]
        )
        ...
        train_df = pd.DataFrame(
            scaler.transform(Z_tr[:, keep]),
            columns=columns,
        )

        train_df["T"] = time[idx_tr]
        train_df["E"] = event[idx_tr]

        test_df = pd.DataFrame(
            scaler.transform(Z_te[:, keep]),
            columns=columns,
        )

        best = max(
            penalizers,
            key=lambda pen: np.mean(
                k_fold_cross_validation(
        ...
        te_time = time[idx_te]
        te_event = event[idx_te]

        rows.append({
            "auc_5y_classifier": roc_auc_score(
                death_before_horizon[idx_te][te_known_mask],
                p_death_classifier[te_known_mask],
            ),
            "auc_5y_cox": roc_auc_score(
                death_before_horizon[idx_te][te_known_mask],
                p_death_cox[te_known_mask],
            ),

            # concordance_index expects larger scores
            # to correspond to longer survival.
            "cindex_classifier": concordance_index(
                te_time,
                1 - p_death_classifier,
                te_event,
            ),
            ...
            "cox_penalizer": best,
            "train_censored_before_horizon_excluded_by_classifier": int(
                (~known[idx_tr]).sum()
            ),
        })
    ...
    out = {
        ...
        "cox_penalizer_mean": float(
            col("cox_penalizer").mean()
        ),
    }

    for metric in ("auc_5y", "cindex"):
        clf = col(f"{metric}_classifier")
        cox = col(f"{metric}_cox")

        diff = cox - clf

        out[metric] = {
            "classifier_mean": float(clf.mean()),
            "classifier_std": float(
                clf.std(ddof=1)
            ),
            "cox_mean": float(cox.mean()),
            "cox_std": float(
                cox.std(ddof=1)
            ),
